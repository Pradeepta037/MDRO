...
#        if self.memory_counter> self.memory_size / 2 and self.memory_counter % self.training_interval == 0:
    ...
    def learn(self):
        ...
        hist = self.gb_classifier.fit(h_train, m_train)

        # You may want to adjust the code below based on your specific use case
        self.cost = hist.history['loss'][0]  # Adjust this line based on your requirements
        assert(self.cost > 0)
        self.cost_his.append(self.cost)
    ...
    def decode(self, h, k = 1, mode = 'OP'):
        ...
        # Use the trained gradient boosting classifier for prediction
        m_pred = self.gb_classifier.predict(h)

        if mode == 'OP':
            return self.knm(m_pred[0], k)
        elif mode == 'GB':  # Update this line to use gradient boosting
            return self.gradient_boosting(m_pred[0], k)
        else:
            print("The action selection must be 'OP' or 'GB'")
    ...
    def gradient_boosting(self, m, k=1):
        # Modify this method based on how you want to implement gradient boosting for your problem
        # You may need to adjust parameters and use the trained model for prediction
        # Use the trained gradient boosting classifier for prediction
        m_pred = self.gb_classifier.predict_proba(m.reshape(1, -1))[0]

        # Select the top k binary offloading actions based on the predicted probabilities
        top_k_indices = np.argsort(m_pred)[::-1][:k]
        top_k_actions = self.enumerate_actions[top_k_indices]
