...
class MemoryCNN:
    def __init__(
        ...
        self.memory = np.zeros((self.memory_size, self.input_shape[0] + self.input_shape[-1]))
    ...
    def _build_net(self):
        self.model = keras.Sequential([
            layers.Conv1D(32, kernel_size=3, activation='relu', input_shape=self.input_shape),
        ...
        self.model.compile(optimizer=keras.optimizers.Adam(lr=self.lr),
                           loss=tf.losses.binary_crossentropy, metrics=['accuracy'])
